...
info.getInfo(code, (Data)=> {
	...
	fileData.chunks.forEach(function(data){
		var fileSavePath = path.join('C:' , 'Users' , 'mohammada', 'Downloads/'+fileData.fileName+'_'+data.chunk);
...
function loggSync (fileData){
	...
	fileData.chunks.forEach(function(data){
		...
		var filePath = path.join('C:' , 'Users' , 'mohammada', 'Downloads/'+fileName+'_'+data.chunk);
		var c = fs.readFileSync (filePath, { encoding: null });
		bodyChunks.push(c);
		console.log(c.length);
		if (data.chunk === 'last') {
			var content = Buffer.concat(bodyChunks)
			fs.writeFileSync(path.join('C:' , 'Users' , 'mohammada', 'Downloads/'+fileName), content);
			var chunkPath = path.join('C:' , 'Users' , 'mohammada', 'Downloads/'+fileName+'_'+data.chunk);
			fs.unlink (chunkPath, ()=>{
							console.log('deleted chunkn file ' + chunkPath);
							});
...
// async buffers
 function loggBuf (fileData){
	...
	var fileName = fileData.fileName;
	var resultingFile = path.join('C:' , 'Users' , 'mohammada', 'Downloads/'+fileName);
